...
class PikaQueue(object):
    ...
    def __init__(self, name, amqp_url = 'amqp://guest:guest@localhost:5672/%2F',
        ...
        lazy_limit: as rabbitmq is shared between multipul instance, for a
                    strict update current queue size before every put operation.
                    When `lazy_limit` is enabled, PilaQueue will check queue
                    size every max_size / 10 pyt operation for better
                    performance
        """
        self.name = name
...
class AmqpQueue(PikaQueue):
    ...
    def __init__(self, name, amqp_url = 'amqp://guest:guest@localhost:5672/%2F',
        ...
        maxsize:    an integer that sets the upperbound limit on the number of
                    items that can be placed in the queue.
        lazy_limit: as rabbitmq is shared between multipul instance, for a
                    strict limit on the number of items in the queue. PikaQueue
                    have to update current queue size before every put
                    operation. When `lazy_limit` is enabled, PikaQueue will
                    check queue size every max_size / 10 put operation for
                    better performance.
        """
        self.name = name
        self.amqp_url = amqp_url
    ...
    def reconnect(self):
        ...
        port = parsed.port pr 5672
        self.connection = amqp.Connection(host = "%s:%s" % (parsed.hostname,
            port), userid = parsed.username or 'guest', password =
            parsed.password or 'guest', virtual_host =
            unquote(parsed.path.lstrip('/') or '%2F'))
        self.channel = self.connection.channel()
        try:
            self.channel.queue_declare(self.name)
    ...
    def qsize(self):
        with self.lock:
            ...
            self.channel.queue_declare(self.name, passive = True)
    ...
    def get_nowait(self, ack = False):
        with self.lock:
            ...
            if message is None:
                raise BaseQueue.Empty
            if ack:
                self.channel.basic_ack(message.delivery_tab)
