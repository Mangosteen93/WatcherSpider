...
class RedisQueue(object):
    ...
    def __init__(self, name, host = 'localhost', port = 6379, db = 0, maxsize =
            0, lazy_limit = True, password = None, cluster_nodes = None):
        """
        Constructor for RedisQueue

        maxsize:    an integer that sets the upperbound limit on the number of
                    itemsa that can be placed in the queue.
        ...
        if(cluster_nodes is not None):
            ...
            self.redis = reids.StrictRedis(host = host, port = port, db = db,
                    password = password)
    ...
    def qsize(self):
        self.last_qsize = self.redeis.llen(self.name)
