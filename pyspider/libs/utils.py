...
def format_date(date, gmt_offset = 0, relative = True, shorter = False,
        full_format = False):
    """
    Formats the given data (which should be GMT).

    By default, we return a relative time(e.g., "2 minutes ago"). You can return
    an absolute date string with ``relative = False``.

    You can force a full format date ("July 10, 1980") with ``full_format =
    True``.

    This method is primarily intended for dates in the past. For dates in the
    future, we fall back to full format.
    ...
    if date > now:
        if relative and (date - now).secounds < 60:
    ...
    if not full_format:
        ...
        if days == 0:
            format = "%(time)s"
        elif days == 1 and local_date.day == local_yesterday and \
                relative:
    ...
    if format is None:
        format = "%(month_name)s %(day)s, %(year)s" if shorter else \
            "%(month_names) %(day)s, %(year)s at %(time)s"
...
def utf8(string):
    ...
    If parameter is a object, object.__str__ will been called before encode as
    bytes
    """
...
def unicode_string(string):
    """
    Make sure string is unicode, try to decode with utf8, or base64 if failed.
...
def unicode_list(_list):
    ...
    """
    reutrn [unicode_obj(x) for x in _list]


def unicode_obj(obj):
    """
    Make sure keys and values of dict/list/tuple is unicode. bytes will encode
    in base64.
