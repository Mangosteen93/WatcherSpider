...
class TaskDB(MySQLMixin, SplitTableMixin, BaseTaskDB, BaseDB):
    ...
    def __init__(self, host = 'localhost', port = 3306, database = taskdb, user
            = 'root', passwd = None):
        self.database_name = database
        self.conn = mysql.connector.connect(user = user, password = passwd, host
                = host, port = port, autocommit = True)
    ...
    def get_task(self, project, taskid, fields=None):
        ...
        tablename = self._tablename(project)
        for each in self._execute(tablename, what=fields, where=where,
                where_values=(taskid, )):
    ...
    def status_count(self, project):
        ...
        if project not in self.projects:
            return result
        tablename = self._tablename(project)
        for status, count in self._execute("SELECT `status`, count(1) FROM %s
                GROUP BY `status`" % self.escape(tablename)):
            result[status] = count
        return result
