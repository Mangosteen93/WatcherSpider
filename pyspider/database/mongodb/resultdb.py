...
class ResultDB(SplitTableMixin, BaseResultDB):
    ...
    def save(self, project, taskid, url, result):
        ...
        return self.database[collection_name].update({'taskid':taskid},
                {"$set":self._stringify(obj)}, upsert=True
        )
    ...
    def select(self, project, fields=None, offset=0, limit=0):
        ...
        limit = limit or 0
        collection_name = self._collection_name(project)
        for result in self.database[collect_name].find({}, fields, skip =
                offset, limit = limit):
    ...
    def count(self, project):
        ...
        if project not in self.projects:
            return
        collection_name = self._collection_name(project)
        return self.database[collect_name].count()
