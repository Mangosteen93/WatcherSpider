...
class ResultDB(SQLiteMixin, SplitTableMixin, BaseResultDB, BaseDB):
    ...
    def _create_project(self, project):
        ...
        self._execute('''CREATE TABLE IF NOT EXISIS `%s` (
                taskid PRIMARY KEY,
                url,
                result,
                updatetime
    ...
    def select(self, project, fields = None, offset = 0, limit = None):
        ...
        for task in self._select2dic(tablename, what=fields, order = 'updatetime
    ...
    def count(self, project):
        ...
        if project not in self.projects:
            return 0
        tablename = self._tablename(project)
        for count, in self._execute("SELECT count(1) FROM %s" %
    ...
    def get(self, project, taskid, fields=None):
        ...
        for task in self._select2dic(tablename, what=fields, where = where,
                where_values=(taskid, )):
