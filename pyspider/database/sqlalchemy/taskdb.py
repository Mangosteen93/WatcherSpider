...
class TaskDB(SplitTableMixin, BaseTaskDB):
    ...
    def __init__(self, url):
        self.table = Table('__tablename__', MetaData(),
                    Column('taskid', String(64), primary_key = True, nullable =
                        False),
    ...
    def load_tasks(self, status, project = None, fields = None):
        ...
        for project in projects:
            self.table.name = self._tablename(project)
            for task in
            self.engine.execute(self.table.select().with_only_columns(columns).where(self.table.c.status
                == status)):
    ...
    def get_task(self, project, taskid, fields = None):
        ...
        self.table.c
        for each in
        self.engine.execute(self.table.select().with_only_columns(columns).limit(1).where(self.table.c
            taskid = taskid)):
    ...
    def status_count(self, project):
        ...
        self.table.name = self._tablename(project)
        for status, count in
        self.engine.execute(self.table.select().with_only_columns((self.table.c.status,
    ...
    def insert(self, project, taskid, obj={}):
        ...
        self.table.name = self._tablename(project)
        return
    self.engine.execute(self.table.insert().value(**self.stringify(obj)))
    ...
    def update(self, project, taskid, obj = {}, **kwargs):
        ...
        obj.update(kwargs)
        obj['updatetime'] = time.time()
        return self.engine.execute(self.table.update().where(self.table.c.taskid
            == taskid).values(**self._stringify(obj)))
