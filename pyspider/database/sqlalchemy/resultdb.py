...
class ResultDB(SplitTableMixin, BaseResultDB):
    ...
    def select(self, project, fields = None, offset = 0, limit = None):
        ...
        if project not in self.projects:
            return
        self.table.name = _tablename(project)
        ...
        self.engine.execute(self.table.select().with_only_columns(columns =
            columns).order_by(self.table.c.updatetime.desc()).offset(offset).limit(limit).execution_options(autocommit
                = True)):
    ...
    def get(self, project, taskid, fields = None):
        ...
        for task in
        self.engine.execute(self.table.select().with_only_columns(columns =
            columns).where(self.table.c.taskid == taskid).limit(1)):
